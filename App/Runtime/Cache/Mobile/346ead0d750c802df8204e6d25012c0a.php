...
<head>
    <title></title>
    <meta name="keywords" content="<?php echo ($cat_name); ?>" />
...
<body>
        ...
        <section class="n_list"> 
            <div>
                <h2><em></em><?php echo ($cat_name); ?></h2>
            </div>
            <ul class="n_list_n clearfix">
            <?php if(is_array($goods_list)): foreach($goods_list as $key=>$vo): ?><li>
                    <a href="<?php echo C('DOMAIN'); echo U('Goods/goodsInfo',array('id'=>$vo[goods_id]));?>">
                     <div><img src="<?php echo ($vo["original_img"]); ?>" alt=""><span><?php echo mb_substr($vo['goods_name'],0,26,'utf-8');?></span></div>  
                    </a>
                </li><?php endforeach; endif; ?>
            </ul>
        </section>
        <section class="page_c">
                <div><a <?php if($_GET['p'] <= 1){echo 'style="display:none;"';};?> href="<?php echo ($prev); ?>">上一页</a></div>
                <div><?php echo ($current); ?>/<?php echo ($num); ?></div>
                <div><a <?php if($_GET['p'] >= $num || $num == 1){echo 'style="display:none;"';};?> href="<?php echo ($next); ?>">下一页</a></div>
