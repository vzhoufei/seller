		...
		<section class="top_nav">
			<div class="top_nav_bar">
				<ul>
					...
					<?php if($nav_where): echo U('Index/index'); else: echo U('Store/index',array('store_id'=>$store[store_id])); endif; ?>
					...
					><a href="
					<?php if($nav_where): echo U('Goods/goods_list'); else: echo U('Store/goods_list',array('store_id'=>$store[store_id])); endif; ?>
					">供应产品 ▼</a>
							<div class="SupplyProducts">
								<ul>
									...
									<?php if($nav_where): echo U('Goods/goods_list',array('cat_id'=>$v[cat_id]));?>
									<?php else: ?>
									...
									<span style="display: none;" class="xs<?php echo ($v['cat_id']); ?> xs"><span><a target="_blank"  style="color:#008BB9;" href="
										<?php if($nav_where): echo U('Goods/goods_list',array('cat_id'=>$vs[cat_id]));?>
									<?php else: ?>
					...
					<!-- 自定义导航 -->
			           <?php if(is_array($navigation)): foreach($navigation as $kk=>$vv): ?><li <?php if($vv['sn_id'] == $_GET['sn_id']){echo 'class="top_nav_barM"';}?> >
			        	...
			        	<?php echo U('Cover/cover',array('sn_id'=>$vv[sn_id]));?>
			        	<?php else: ?>
      	...
      	<!-- 分类在导航显示 -->
		      	<?php if(is_array($link_cat)): foreach($link_cat as $key=>$vz): ?><li <?php if($vz['cat_id'] == $_GET['cat_id']){echo 'class="top_nav_barM"';}?>>
					...
					<?php if($nav_where): echo U('Goods/goods_list',array('cat_id'=>$vz[cat_id]));?>
					<?php else: ?>
